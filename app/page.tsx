import { ArrowRight, ArrowUpRight, Clock, FileText, Layers, Trees } from "lucide-react";
...
// in from `content/`.
const WORDMARK = "Rökland";
const HERO_EYEBROW = "En guide till vår del av Alnö";
const HERO_TAGLINE =
  "Från öns urgamla berggrund och Alnö gamla kyrka till sågverkens tid och en splitter ny stadsdel. Historia, natur och kuriosa — ett kapitel i taget.";
const CTA_LABEL = "Börja läsa";
const FOOTER_NOTE = "Rökland — historien, naturen och guiden till vår del av Alnö.";

// ── Fler kurser ───────────────────────────────────────────────────────────
// Manuellt kuraterad lista över de andra kurssajterna i familjen. Uppdatera
// listan på varje sajts startsida när en ny kurs skeppas.
const OTHER_COURSES = [
  {
    title: "Fuel Lab",
    href: "https://nutrition-course-eight.vercel.app",
    description: "Evidensbaserad näringslära för idrottare — på engelska.",
  },
  {
    title: "Psoriasis & PsA",
...
export default function Home() {
  ...
  const stats = [
    { icon: FileText, label: `${chapters.length} kapitel` },
    { icon: Clock, label: `~${minutes} min läsning` },
    { icon: Layers, label: `${sections.length} avsnitt` },
  ];
  ...
  return (
    <main className="relative flex flex-1 flex-col overflow-hidden">
      ...
      <div aria-hidden className="pointer-events-none absolute inset-0 -z-10">
        ...
        <Trees className="absolute -right-24 top-24 icon-96 rotate-12 text-brand/[0.07]" />
      </div>
      ...
      <header className="mx-auto flex w-full max-w-6xl items-center justify-between px-6 py-5 md:px-10">
        <span className="flex items-center gap-2 text-base font-semibold text-brand-ink">
          <Trees className="icon-5 text-brand" />
          {WORDMARK}
        </span>
        <ThemeToggle />
      </header>
      ...
      <section className="mx-auto flex w-full max-w-6xl flex-col items-center px-6 pb-20 pt-16 text-center md:px-10 md:pt-24">
        ...
        >
          Berättelsen om <span className="text-brand">Rökland</span>.
        </h1>
        <p
        ...
        >
          ...
          <Button asChild variant="ghost" size="lg">
            <Link href="#chapters">Bläddra bland kapitlen</Link>
          </Button>
        </div>
      ...
      {/* Andra kurser i familjen — en manuellt kuraterad lista. */}
      {OTHER_COURSES.length > 0 && (
        <section className="mx-auto w-full max-w-6xl px-6 pb-20 md:px-10">
          ...
          <p className="mt-2 text-ink/60">
            Samma recept, andra ämnen — fler små kurser vi byggt.
          </p>
